/*************  ✨ Codeium Command ⭐  *************/
import { useEffect, useState } from "react";
import { Form } from "react-bootstrap";
import { Axios } from "../../../Api/Axios";
import Loading from "../../../Componenets/Loading/Loading";
import { useNavigate, useParams } from "react-router-dom";
import { CAT, Product } from "../../../Api/Api";
import { useRef } from "react";
import '../dashboard.css';
import { Button } from "react-bootstrap";
export default function UpdateProduct() {

    const [image, setImage] = useState([]);
    const [oldImage, setOldImage] = useState([]);
  

    const [oldImageID, setOldImageID] = useState([]);
    const [loading, setLoading] = useState(false);


    const [categories, setCategories] = useState([]);

    const { id } = useParams();


    console.log("oldimageid" + oldImageID);


 const save = useRef(false);

    
    console.log(save.current)

    const nav = useNavigate();


    const [FormState, setFormState] = useState({

        title: "",
        description: "",
        category: "Select Category",
        About: "",
        price: "",
        discount: "",

    });




    //get all categories
    useEffect(() => {
        Axios.get(CAT).then((res) => {
            setCategories(res.data);
        })
    }, []);


    //useref
    const inputRef = useRef(null);

    const deleteImagebyID = useRef([]);
    const progress = useRef([]);
    const j = useRef(-1);


    const OpenImage = useRef(null);
    // image upload function
    function handleUploadImage() {
        OpenImage.current.click();
    };

    //fucus 
    useEffect(() => {
        if (inputRef.current) {
            inputRef.current.focus();
        }
    }, []);

    // get All Categories 

    useEffect(() => {

        Axios.get(`/${CAT}`)
            .then((res) => {
                setCategories(res.data)
                console.log(res)
            })
            .catch((err) => console.log(err))

    }, []);

    const categoriesShow = categories.map((item, key) =>
        <option key={key} value={item.id}>{item.title}</option>
    );



    // handel submit form 
    /* const HandelSubmitForm = async () => {
            try {
                const res = await Axios.post(`${Product}/add`, dumyForm);
                setId(res.data.id)
                console.log(res.data.id)
                       
            } catch (err) {
                console.log(err)
               
            }
        }*/




    useEffect(() => {
        (async () => {
            setLoading(true);
            try {
                const res = await Axios.get(`${Product}/${id}`);
                console.log()

                setFormState(res.data[0])

                setOldImage(res.data[0].images);




                setLoading(false);



            } catch (error) {
                setLoading(false);
                console.log(error);

            }
        })();
    }, [id]);

    if (loading) {
        return <Loading />;
    }


    // HandelEdit

 async function HandelEdit(e) {
        
      
        e.preventDefault();
        console.log(oldImageID.length);
       
 
            
        
  

         

       

  

         
          setLoading(true);
        try {     
             if(oldImageID.length >= 0) 
                {  for (let i = 0; i < oldImageID.length; i++) {
                const res = await Axios.delete(`/product-img/${oldImageID[i]}`);
                console.log(res);
            }}
            const res = await Axios.post(`${Product}/edit/${id}`, FormState);
             
              nav("/dashboard/products");

        } catch (err) {
            console.log(err)
            setLoading(false);
        }
    }
    //handle form change

    const handleChange = (e) => {
        setFormState({ ...FormState, [e.target.name]: e.target.value });

    }

    // handle image change
    /*  async function handeleImageChange(e) {
  
          setImage((prev) => [...prev, ...e.target.files]);
          const ImageAsFile = e.target.files;
          const data = new FormData();
          for (let i = 0; i < ImageAsFile.length; i++) {
              j.current++;
  
              data.append('image', ImageAsFile[i]);
              data.append('product_id', id);
  
              try {
  
                  const res = await Axios.post("product-img/add", data, {
  
                      onUploadProgress: (ProgressEvent) => {
  
                          const { loaded, total } = ProgressEvent;
                          const percent = ((loaded * 100) / total);
                          if (percent % 10 === 0) {
                              progress.current[j.current].style.width = `${percent}%`;
                          }
  
                      },
  
                      // GET ID OF SINGLE IMAGE
  
                  },
  
                  );
  
                  deleteImagebyID.current[j.current] = res.data.id;
                  console.log(deleteImagebyID.current[j.current]);
  
              } catch (error) {
  
  
                  console.log(error)
              }
          }
      }*/

    async function handeleImageChange(e) {
        console.log("1" + j.current);
        setImage((prev) => [...prev, ...e.target.files]);
        const ImageAsFile = e.target.files;
        const data = new FormData();
        for (let i = 0; i < ImageAsFile.length; i++) {
            j.current++;
            console.log("2" + j.current);
            data.append('image', ImageAsFile[i]);
            data.append('product_id', id);

            try {

                const res = await Axios.post("product-img/add", data, {

                    onUploadProgress: (ProgressEvent) => {

                        const { loaded, total } = ProgressEvent;
                        const percent = ((loaded * 100) / total);
                        console.log(progress.current[j.current])
                        if (percent % 10 === 0) {
                            console.log("3" + j.current);
                            progress.current[j.current].style.width = `${percent}%`;
                        }

                    },
                    // GET ID OF SINGLE IMAGE           
                },

                );

                deleteImagebyID.current[j.current] = res.data.id;
                console.log(deleteImagebyID.current[j.current]);

            } catch (error) {


                console.log(error)
            }
        }

    }


    async function handeleDeleteImage(id, img) {
        try {

            console.log("handeleDeleteImage id " + deleteImagebyID.current[id]);

            setLoading(true)
            const res = await Axios.delete(`product-img/${deleteImagebyID.current[id]}`)
            setImage((prev) => prev.filter((imgs) => imgs !== img));
            deleteImagebyID.current = deleteImagebyID.current.filter((imgs) => imgs !== deleteImagebyID.current[id]);
            --j.current;

            setLoading(false)

        } catch (error) {
            console.log(error)
            setLoading(false)

        }

    }

    async function handeleDeleteImageFromEdit(id) {

        console.log("save", save);
         
        console.log(oldImageID.length);

        setOldImage((prev) => prev.filter((imgs) => imgs.id !== id));

        setOldImageID((prev) => { return [...prev, id] });



  

    }

    // show images
    const ShowOldImages = oldImage.map((img, key) => (


        <div key={key} className="border p-2 ">

            <div className="d-flex align-items-center justify-content-start gap-2">
                <img src={img.image} alt="" width={"170px"} height={"140px"} />
                <div className="progress position-absolute top-0 start-0" style={{ height: "3px" }}></div>

            </div>

            <Button style={{
                width: "60px", height: "30px",
                fontSize: "small", padding: "5px"

            }}
                variant="danger"
                onClick={() => handeleDeleteImageFromEdit(img.id)}
            >Delete</Button>

        </div>

    ))
    const ShowImages = image.map((img, key) =>


        <div key={key} className="border p-2 ">

            <div className="create-progress mb-2 ">
                <span ref={(e) => (progress.current[key] = e)}
                    className=" progress-bar"
                >
                </span>
            </div>
            <div>

                <div className="d-flex align-items-center justify-content-start gap-2">
                    <img src={URL.createObjectURL(img)} alt="" width={"170px"} height={"140px"} />
                    <div className="progress position-absolute top-0 start-0" style={{ height: "3px" }}></div>

                </div>
                <p>{(img.size / 1024) < 900 ? (img.size / 1024).toFixed(2) + "KB" :
                    (img.size / 1024 / 1024).toFixed(2) + "MB"}</p>

                <Button style={{
                    width: "60px", height: "30px",
                    fontSize: "small", padding: "5px"

                }}
                    variant="danger"
                    onClick={() => handeleDeleteImage(key, img)}
                >Delete</Button>

            </div>



        </div>


    )



    return (
        <>

            {loading && <Loading />}
            <Form className=" mx-2 border w-100 p-3" onSubmit={HandelEdit}>

                {/* create form group to category */}

                <Form.Group className="mb-3" controlId="category" >
                    <Form.Label>Category</Form.Label>
                    <Form.Select
                        type="text"
                        value={categories}
                        required
                        name="category"
                        onChange={handleChange}


                    >
                        <option selected disabled >Select Category</option>
                        {categoriesShow}

                    </Form.Select>
                </Form.Group>


                <Form.Group className="mb-3  " controlId=" title">
                    <Form.Label>Title</Form.Label>
                    <Form.Control type="text"
                        placeholder="title..."
                        value={FormState.title}
                        required
                        name="title"
                        onChange={handleChange}


                    />
                </Form.Group>

                {/* create form group to description */}

                <Form.Group className="mb-3 custom-form-group" controlId="description">
                    <Form.Label>Description</Form.Label>
                    <Form.Control as="textarea" rows={3}
                        placeholder="description..."
                        value={FormState.description}
                        name="description"
                        onChange={handleChange}

                    />

                </Form.Group>



                {/* create form group to about */}
                <Form.Group className="mb-3 custom-form-group" controlId="about">
                    <Form.Label>About</Form.Label>
                    <Form.Control as="textarea" rows={3}
                        placeholder="About..."
                        value={FormState.About}
                        name="About"
                        onChange={handleChange}
                    />

                </Form.Group>
                {/* create form group to price */}
                <Form.Group className="mb-3" controlId="price">
                    <Form.Label>Price</Form.Label>
                    <Form.Control
                        type="number"
                        placeholder="price..."
                        value={FormState.price}
                        name="price"
                        onChange={handleChange}


                    />
                </Form.Group>

                {/* create form group to discount */}
                <Form.Group className="mb-3" controlId="discount">
                    <Form.Label>Discount</Form.Label>
                    <Form.Control type="number"
                        placeholder="discount..."
                        value={FormState.discount}
                        name="discount"
                        onChange={handleChange}

                    />
                </Form.Group>

                {/* create form group to images */}

                <Form.Group className="mb-3" controlId="image">
                    <Form.Label>Image</Form.Label>
                    <Form.Control
                        ref={OpenImage}
                        hidden
                        type="file"
                        multiple
                        onChange={handeleImageChange}
                    />
                </Form.Group>
                <div>
                    <div
                        onClick={handleUploadImage}
                        className=" 
                       d-flex justify-content-center align-items-center flex-column mb-3"
                        style={{ border: "3px dashed #528ac1", cursor: "pointer" }}
                    >

                        <img src={require("../../../Assets/UploadImage.png")}
                            width={"100px"}

                        />

                        <p style={{ color: "#528ac1 ", fontWeight: "bold" }}>Upload Image Her</p>



                    </div>



                </div>


                {loading && (
                    <tr>
                        <td colSpan="4" className="text-center">
                            <Loading />
                        </td>
                    </tr>
                )}
                <div
                    style={{ flexWrap: "wrap" }}
                    className="gap-2 d-flex mb-3 w-100">{ShowImages}
                </div>
                <div
                    style={{ flexWrap: "wrap" }}
                    className="gap-2 d-flex mb-3 w-100">{ShowOldImages}
                </div>
                <button disabled={
                    FormState.title.length > 1 ?
                        false : true}
                    type="submit"
                    style={{
                        width: "100px",
                        height: "40px",
                        fontSize: "small",
                        padding: "5px",
                        marginTop: "20px"
                    }}




                    className="btn btn-primary ">Save</button>
            </Form>
        </>

    )
} 
